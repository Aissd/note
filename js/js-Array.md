...
5、查找元素：indexOf()是严格匹配

6、查找元素：includes() - 对引用类型的（对象，数组）无效
...
7、查找元素：find() - 就近原则，找到即返回那个要找的元素的值，对引用类型（对象，数组）有效
...
10、遍历：array.forEach(function(value, index, array) {}); - 可直接操作DOM元素
...
12、every() - 当遍历的全部为真时，返回true，当有一个为false时，返回false，后面就不遍历了

13、some() - 有一个为真，即返回true，后面就不遍历了
